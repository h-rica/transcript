...
pub fn ModelManagerPage() -> impl IntoView {
    ...
    view! {
        <WorkspaceShell route=WorkspaceRoute::Models>
            <WorkspaceHeader
                title="Models"
                subtitle="Inspect hardware fit, storage pressure, and local model availability before switching transcription engines."
            >
            ...
            <section class="rounded-[1.15rem] border border-zinc-200 bg-zinc-100/70 px-4 py-4 dark:border-zinc-900 dark:bg-[#15171b]">
            ...
            <section class="space-y-3 rounded-[1.15rem] border border-zinc-200 bg-white px-4 py-4 dark:border-zinc-900 dark:bg-[#141519]">
                <div class="flex flex-wrap items-center justify-between gap-3 text-sm text-zinc-600 dark:text-zinc-500">
                    ...
                    <span>
                        {move || {
                            ...
                            format!("{} MB budgeted · local model store", total)
                        }}
                    </span>
                </div>
                                    ...
                                    <div class="flex flex-wrap items-start justify-between gap-4">
                                        <div class="min-w-0 flex-1">
                                            <div class="flex flex-wrap items-center gap-2">
                                                <h2 class="text-sm font-semibold text-zinc-950 dark:text-zinc-100">{model.name.clone()}</h2>
                                                ...
                                                <Show when=move || !is_ready>
                                                    <StatePill label="Missing" tone="muted"/>
                                                </Show>
                                            </div>
                                            <p class="mt-2 text-sm leading-6 text-zinc-600 dark:text-zinc-400">{model.description.clone()}</p>
                                            <div class="mt-3 flex flex-wrap items-center gap-2 text-[11px] text-zinc-500 dark:text-zinc-500">
                                                <span class="inline-flex items-center rounded-full border border-zinc-200 px-2.5 py-1 dark:border-zinc-800">{format!("{} MB", model.size_mb)}</span>
                                                <span class="inline-flex items-center rounded-full border border-zinc-200 px-2.5 py-1 dark:border-zinc-800">{model.source.clone()}</span>
                                                <span class="inline-flex items-center rounded-full border border-zinc-200 px-2.5 py-1 dark:border-zinc-800">{model.languages.join(" + ")}</span>
